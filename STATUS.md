...
must avoid. Skeptic pass queued (new top item). Also: the deep-anneal
...
1. [union-closed] ~~Attack both surviving Gap-1 candidates (unsigned |σ|-control; λ-window variant; assembly restatement)~~ **DONE in 020 — ALL REFUTED, certified** (see TL;DR; swarm-guided designs, director-certified; 018's queue items (a), (b), (c) all settled negatively: the |σ|-control dies via the non-PSD sensitivity kernel at n = 4–5, the window variant dies via a₁-cancellation measures with negative a₂ at n = 4, and the assembly's aggregate secant requirement is violated in-window, with R₊-pure witnesses ruling out every chain-rule-compatible per-history weighting). Replacement, in order: (a) **skeptic pass on 020** (mandatory; new exact machinery: `exact_mm_abs`'s dz/dρ interval step, the part-B/F certificates — re-derive with an independent enclosure kit, e.g. 017's interval-squaring path; re-run the three part-B tilts and the R₊ z < 1/2 legs; cross-family rule: this record's analysis inputs were gpt-heavy, so a gemini-drafted or swarm-free skeptic); (b) **run 020's two witness geometries against Gap 2's CR/TAX/ST** (009/011 engines; if the tax candidate survives the geometries that killed Gap 1 its mechanism is genuinely different — 020 lead 4); (c) **certify the m = 2 tensor at n = 8** (020 lead 2, upgrades the every-n window kill from SPECULATION); (d) **map the a₁-cancellation manifold** (020 lead 5: which genres have negative-a₂ points; closed-form a₂ on the manifold would settle the tensor claim without computation).
