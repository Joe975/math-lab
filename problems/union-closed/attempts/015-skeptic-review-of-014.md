# 015 — Skeptic review of 014 (aggregated control killed at large n): independent exact recomputation

- **Problem:** union-closed, `problems/union-closed/PROBLEM.md`
- **Date:** 2026-08-04
- **Mode:** informed
- **Type:** adversarial verification of `014-aggregated-control-probe-large-n.md`
  (default stance: refute the refutation — a wrong kill would close a live
  proof branch incorrectly). 014's certificates reuse 013's exact engine and
...
- **Outcome in one line:** **014 holds — the kill stands.** All three kill
  ...
  is dead; the λ-restricted restatement (014's own lead 4) is more alive
  than the record's P4 framing suggests.
- **Tools:** `explore/uc_or_agg_skeptic.py` (written here; stdlib only;
  imports NOTHING from `uc_or_avg*.py` / `uc_or_agg_probe*.py`; instance
  parameters are read as data from 014's checkpoints, and the MU(n,r)
  builder is re-implemented from 014 §1's prose then cross-checked
  ...
  `python problems/union-closed/explore/uc_or_agg_skeptic.py --n128 --n160`
...
- **Sources:** 007/013 (definition of M_i, w_i, the aggregate, the anchors),
  014 (the record under attack and its checkpoints), 005 (Prop 1 anchor),
...
1. **Quantity fidelity** (highest value): is 014's A(μ,λ) exactly the
...
- **C1 (reporting).** The n = 128 tidy-rational certificate instance has
  **252 atoms** (2r + 8 with r = 122), not the "253 atoms" printed in 014's
...
- **C2 (design framing, the one that matters downstream).** P4's sentence
  ...
  λ-restricted regime the assembly actually uses at these n, so 014's lead
...
- **Normalization robustness:** w_i is the globally-normalized defined
  ...
  47.9006 (matches 014's "≈ 47.9"); per-i census structure matches the
...
- **Padding invariance as an exact identity:** MU(20,1) and MU(32,1) at
  ...
  (identical rational bounds, identical W/Z) — 014's part-A anchor holds
  not to 0.00e+00 float but as exact rational equality.

### 2. Family validity (claim 2) — CONFIRMED

- **Construction:** my MU(n,r) builder, written from 014 §1's prose
...
Each interval **contains 014's certified enclosure** (n = 96:
...
- **P3-best n = 32** (tidy, zero-dropped, 36 atoms): my engine certifies
  +0.297679573398 at t = 4, dichotomy exact — matching 014's re-cert and
...
- **"Surviving positive space" is real:** the RAW-witness-weight ladder at
  ...
  **+0.157131854664** at t = 4 — my certificate upgrades 014's float
...
- Search-completeness claims (n ≤ 32 clean, 58+6 climbs, permutation
  ...
  re-run — they are scoped as EVIDENCE in 014 and carry no weight in the
...
**Net assessment:** 014's headline — the i-aggregated odds-ratio control
...
- **Search-completeness (n ≤ 32 positive space)** was audited for
  checkpoint consistency, not re-run. It is EVIDENCE in 014 and remains
  EVIDENCE.
...
- Reviewed record: `problems/union-closed/attempts/014-aggregated-control-probe-large-n.md`;
  its tools `explore/uc_or_agg_probe.py`, `explore/uc_or_agg_probe2.py`;
...
- Context: `attempts/007-averaged-or-control.md` (§1 definition, §5
  ...
  measured λ_max table); `attempts/012` (large-n lesson, cited by 014).
- This review's tool/data: `explore/uc_or_agg_skeptic.py`;
